...
const IDENTITY_TITLE = /(设计师|摄影师|作家|艺术家|工程师|创作者|designer|photographer|writer|artist|engineer|creator)/i;
...
function longestTextLength(card: Card): number {
  return card.blocks.reduce((longest, block) => {
    if (block.type === 'text') return Math.max(longest, block.text.length);
    if (block.type === 'list' || block.type === 'tags') {
      return Math.max(longest, ...block.items.map((item) => item.length));
    }
    if (block.type === 'todo') {
      return Math.max(longest, ...block.items.map((item) => item.text.length));
    }
    return longest;
  }, 0);
}
...
function isDenseNarrativeCard(card: Card): boolean {
  const role = inferJournalRole(card);
  return (role === 'body' || role === 'quote' || role === 'profile')
    && (longestTextLength(card) > 240 || listLikeItemCount(card) > 8);
}
...
function findLocalHeroId(cards: Card[]): string | undefined {
  ...
  const profileImage = cards.find((card) => (
    ...
    && (PROFILE_TITLE.test(card.title) || IDENTITY_TITLE.test(card.title) || /(封面|cover)/i.test(card.title))
  ));
  return profileImage?.id;
}

function findLocalLeadId(cards: Card[], heroId?: string): string | undefined {
  if (heroId) return heroId;
  return sortJournalCards(cards).find((card) => {
    const role = inferJournalRole(card);
    return role === 'profile' || role === 'project' || role === 'body';
  })?.id;
}

function localNarrativeRank(card: Card, heroId?: string, leadId?: string): number {
  if (card.id === heroId) return -10;
  if (card.id === leadId) return -5;
  const role = inferJournalRole(card);
...
function sortCardsForLocalLayout(cards: Card[]): Card[] {
  ...
  const leadId = findLocalLeadId(cards, heroId);
  return [...cards].sort((a, b) => (
    localNarrativeRank(a, heroId, leadId) - localNarrativeRank(b, heroId, leadId)
    || a.y - b.y
    || a.x - b.x
  ));
}
...
function isSuitableFeaturePair(a: Card, b: Card): boolean {
  if (isHeavyTodoCard(a) || isHeavyTodoCard(b)) return false;
  if (isDenseNarrativeCard(a) || isDenseNarrativeCard(b)) return false;
  const aSignal = journalLayoutSignal(a);
  const bSignal = journalLayoutSignal(b);
  return !(aSignal.preferredSpan >= 7 && bSignal.preferredSpan >= 7);
}

function isProjectCard(card: Card): boolean {
  return inferJournalRole(card) === 'project';
}

function isProjectSupportCard(card: Card): boolean {
  const role = inferJournalRole(card);
  return role === 'skills'
    || role === 'note'
    || role === 'quote'
    || role === 'link'
    || (role === 'todo' && !isHeavyTodoCard(card));
}

function isProjectSupportPair(a: Card, b: Card): boolean {
  return (isProjectCard(a) && isProjectSupportCard(b))
    || (isProjectCard(b) && isProjectSupportCard(a));
}

function isClosingNoteLinkPair(a: Card, b: Card): boolean {
  const roles = [inferJournalRole(a), inferJournalRole(b)];
  return roles.includes('note') && roles.includes('link');
}

function isRecapPair(a: Card, b: Card): boolean {
  const roles = [inferJournalRole(a), inferJournalRole(b)];
  return roles.includes('stat')
    && roles.includes('body')
    && /(复盘|总结|回顾|结论|recap|summary|review)/i.test(`${a.title} ${b.title}`);
}
...
function singleCardSpan(card: Card, mode: 'ai' | 'local'): number {
  if (isHeavyTodoCard(card)) return 12;
...
function patternSpans(
  ...
  if (cards.length === 2 && isProjectSupportPair(cards[0], cards[1])) {
    return isProjectCard(cards[0]) ? [7, 5] : [5, 7];
  }
  if (cards.length === 2 && isRecapPair(cards[0], cards[1])) {
    return inferJournalRole(cards[0]) === 'stat' ? [4, 8] : [8, 4];
  }
  if (cards.length === 2 && isClosingNoteLinkPair(cards[0], cards[1])) {
    return inferJournalRole(cards[0]) === 'note' ? [5, 7] : [7, 5];
  }
...
function isSafeTrioCard(card: Card): boolean {
  const signal = journalLayoutSignal(card);
  return signal.preferredSpan <= 5
    && signal.weight <= 6.5
    && longestTextLength(card) <= 96
    && listLikeItemCount(card) <= 6
    && !hasContentImage(card);
}

function isSafeQuartetCard(card: Card): boolean {
  const signal = journalLayoutSignal(card);
  return signal.preferredSpan <= 4
    && signal.weight <= 4.5
    && longestTextLength(card) <= 56
    && listLikeItemCount(card) <= 4
    && !hasContentImage(card);
}
...
function constrainedGroupRows(
  ...
  if (cards.length === 2) {
    if (!isSuitableFeaturePair(cards[0], cards[1])) {
      if (isRecapPair(cards[0], cards[1])) {
        return [{
          cards,
          pattern: inferJournalRole(cards[0]) === 'stat' ? 'focus-right' : 'focus-left',
          align: 'start',
        }];
      }
      return cards.map((card) => ({ cards: [card], pattern: 'single' }));
    }
    if (isProjectSupportPair(cards[0], cards[1])) {
      return [{
        cards,
        pattern: isProjectCard(cards[0]) ? 'focus-left' : 'focus-right',
        align: 'start',
      }];
    }
    if (isClosingNoteLinkPair(cards[0], cards[1])) {
      return [{
        cards,
        pattern: inferJournalRole(cards[0]) === 'note' ? 'focus-right' : 'focus-left',
        align: 'end',
      }];
    }
  ...
  if (cards.length === 3) {
    ...
    return [
      ...
      ...constrainedGroupRows(supports, 'balanced', heroId),
    ];
  }
...
function layoutCardsLocally(cards: Card[], startRow = 0): AutoJournalLayoutResult & { nextRow: number } {
  ...
  while (pending.length) {
    ...
    if (isProjectCard(current)) {
      const laterProjects = pending.filter(isProjectCard);
      if (laterProjects.length) {
        const projects = [current, ...laterProjects].sort((a, b) => a.y - b.y || a.x - b.x);
        for (let index = pending.length - 1; index >= 0; index--) {
          if (isProjectCard(pending[index])) pending.splice(index, 1);
        }
        projects.forEach((project, index) => {
          const nextProject = projects[index + 1];
          const betweenIndex = pending.findIndex((candidate) => (
            isProjectSupportCard(candidate)
            && candidate.y > project.y
            && (!nextProject || candidate.y < nextProject.y)
          ));
          const companionIndex = betweenIndex >= 0
            ? betweenIndex
            : pending.findIndex(isProjectSupportCard);
          if (companionIndex < 0) {
            row = addLayoutRow(layout, orderedCards, [project], 'single', row, 'local', 'start');
            return;
          }
    ...
    const recapIndex = pending.findIndex((candidate, index) => (
      index < 3 && isRecapPair(current, candidate)
    ));
    if (recapIndex >= 0) {
      const [recapCompanion] = pending.splice(recapIndex, 1);
      const rowCards = [current, recapCompanion];
      row = addLayoutRow(
        layout,
        orderedCards,
        rowCards,
        inferJournalRole(current) === 'stat' ? 'focus-right' : 'focus-left',
        row,
        'local',
        'start',
      );
...
function layoutCardsWithAIPlan(
  ...
  const layout = new Map<string, AutoJournalLayout>();
  const orderedCards: Card[] = [];
  const plannedHeroId = plan.groups
    .filter((group) => group.pattern === 'hero' && Array.isArray(group.cardIds))
    .flatMap((group) => group.cardIds)
    .find((id) => byId.has(id));
  const heroId = plannedHeroId ?? findLocalHeroId(cards);
  let row = 0;
    ...
    for (let index = 0; index < groupCards.length; index += capacity) {
      ...
      for (const constrained of constrainedRows) {
        row = addLayoutRow(
          ...
          'ai',
          constrained.align
            ?? (isJournalVerticalAlign(group.align) ? group.align : 'center'),
        );
      }
    }
  }
