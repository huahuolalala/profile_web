# YumMe（YummyMind）· 数字手账

把想法、经历与作品铺成一页真实手账，可导出精美单文件 HTML。React 手账编辑器 + Go/Hertz 后端。旧画布坐标与连线数据仅保留兼容，不再作为主交互。
...
AI 通过生成 DSL 代码来模拟用户添加素材。用户在编辑器顶栏「导入代码」粘贴 DSL，前端校验后按内容顺序铺到手账页面。
...
- `cards[].x / y`：可选，仅用于兼容旧数据和保存阅读顺序；默认不要提供
- `edges`：可选兼容字段，`from`/`to` 是 `cards` 数组的**下标**（0 起）；新内容默认不要生成
...
4. 一张素材只承载一个主题；内容多时拆成多张素材，依靠数组顺序表达阅读节奏
5. 修改现有手账：先让用户提供「导出代码」的 JSON，在其基础上增量修改并输出完整 DSL
